...
export const addtaskController = async (req, res) => {
  try {
    ...
    await axios
      .get("http://host.docker.internal:5000/auth/search/" + assignee)
      .catch((err) => {
    ...
    await axios
      .get("http://host.docker.internal:5000/auth/search/" + assigner)
      .catch((err) => {
        if (err.response.status == 401) {
    ...
    await axios
      .post("http://host.docker.internal:5000/team/checkValidity", requestBody)
      .catch((err) => {
        if (err.response.status == 401) {
    ...
    axios.post("http://host.docker.internal:5003/addnotification", req.body);
